...
class RandomRotation(object):
  ...
  def __call__(self, img, target):
    ...
    angle =  random.uniform(-self.max_angle, self.max_angle) / 10
    M  = cv2.getRotationMatrix2D(center, angle, 1)
    dst_size = (img.shape[1], img.shape[0])
    dst = cv2.warpAffine(img, M, dst_size) #image rotation

    angle_rad = - angle * math.pi / 180

    annotations = target['annotations'] #batch size = 1

    #target coordinate rotation
    for i in range(len(target['annotations'])):

      bbox_gto = annotations[i]['bbox'] 
